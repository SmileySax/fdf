...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/06/16 00:59:02 by keaton            #+#    #+#             */
/*   Updated: 2022/06/16 01:52:29 by keaton           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_menu(t_map *map)
{
	...
	mlx_string_put(m, w, 10, 20
		* indent++, 0x94E411, "3d rotation: Q/W/E/A/S/D");
...
void	ft_drawline(t_map *map, t_point p1, t_point p2)
{
	...
	while ((int)(p1.x - p2.x) || (int)(p1.y - p2.y))
	{
		...
		if (p1.x >= 0 && p1.y >= 0 && p1.x <= (map->win_size)[0]
				&& p1.y <= (map->win_size)[1])
...
void	ft_draw(t_map *map)
{
	...
	ft_bzero((void *)map->img_addr, map->win_size[0]
		* map->win_size[1] * (map->bits_per_pixel / 8));
	y = 0;
	while (y < map->len)
	{
		x = 0;
